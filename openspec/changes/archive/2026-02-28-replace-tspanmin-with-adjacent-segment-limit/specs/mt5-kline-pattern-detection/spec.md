...
#### Scenario: 相邻线段数量超限时拒绝该序列
- **WHEN** 候选序列的 `P0-P1`、`P1-P2`、`P2-P3`、`P3-P4` 四个线段中，bar span 等于 `1` 的线段数量大于 `InpMaxAdjustPointSpan`
- **THEN** 检测器拒绝该序列，且不会报告有效模式匹配
...
策略 SHALL 仅在以下条件全部满足时将某个形态视为完整匹配：`b1 = x * b2` 且落在配置的 `x` 系数范围内，`r1 = y * r2` 且落在配置的 `y` 系数范围内，`t4 < z * (t1 + t2 + t3)`，`c < m * a`，`adjacent_segment_count <= InpMaxAdjustPointSpan`，以及每个相邻点跨度都小于或等于 `AdjustPointMaxSpanKNumber`。这里的 `adjacent_segment_count` SHALL 统计 `P0-P1`、`P1-P2`、`P2-P3`、`P3-P4` 四个线段中 bar span 恰好等于 `1` 的线段数量。
