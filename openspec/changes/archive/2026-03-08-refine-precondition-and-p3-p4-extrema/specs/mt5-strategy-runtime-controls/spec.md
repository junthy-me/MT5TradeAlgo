...
策略 SHALL 暴露 `InpSymbols`、`InpTF`、`InpTimerMillSec`、`InpMagic`、`InpComment`、`InpFixedLots`、`InpMaxPositionsPerSymbol`、`InpSlippagePoints`、`InpProfitObservationBars`、`InpStopObservationBars`、`InpLookbackBars`、`InpAdjustPointMinSpanKNumber`、`InpAdjustPointMaxSpanKNumber`、`InpP1P2AValueSpaceMinPriceLimit`、`InpP1P2AValueTimeMinKNumberLimit`、`InpBSumValueMinRatioOfAValue`、`InpBSumValueMaxRatioOfAValue`、`InpPreCondEnable`、`InpPreCondPriorDeclineLookbackBars`、`InpPreCondPriorDeclineMinDropRatioOfStructure`、`InpPreCondPriorDeclineMinBarsBetweenPre0AndP0` 和 `InpP5AnchoredProfitC` 等运行时输入参数。`InpSymbols` SHALL 接受以分号分隔的交易品种列表，且运行时 SHALL 在解析后忽略空白项。`InpProfitObservationBars` SHALL 使用 bar 数量表示止盈后观察窗口长度；当其为 `0` 时，策略 SHALL 视为禁用止盈观察窗口；当其为正整数时，策略 SHALL 在对应 `symbol + timeframe` 的最近一次 `profit_target` 平仓后，按该 bar 数长度执行观察期门控。`InpStopObservationBars` SHALL 使用相同的 bar 计数语义表示止损后观察窗口长度；当其为 `0` 时，策略 SHALL 视为禁用止损观察窗口；当其为正整数时，策略 SHALL 在对应 `symbol + timeframe` 的最近一次 `hard_stop` 或 `soft_stop` 平仓后，按该 bar 数长度执行观察期门控。`InpAdjustPointMinSpanKNumber` 和 `InpAdjustPointMaxSpanKNumber` SHALL 共同定义相邻点之间中间间隔 K 线数量的最小/最大区间。`InpPreCondEnable` SHALL 控制是否启用 `Pre0-P0` 前置条件；当其为 `false` 时，策略 SHALL 跳过 `Pre0` 搜索与相关过滤；当其为 `true` 时，策略 SHALL 启用 `Pre0-P0` 前置下跌规则。`InpP5AnchoredProfitC` SHALL 作为首次 `P5/P6` 激活后唯一止盈公式的系数输入。策略 SHALL NOT 再暴露 `InpProfitC` 作为运行时输入参数。
