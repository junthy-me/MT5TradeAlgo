...
策略 SHALL 暴露 `InpSymbols`、`InpTF`、`InpTimerMillSec`、`InpMagic`、`InpComment`、`InpFixedLots`、`InpMaxPositionsPerSymbol`、`InpSlippagePoints`、`InpProfitObservationBars`、`InpStopObservationBars`、`InpLookbackBars`、`InpP1P2AValueSpaceMinPriceLimit`、`InpP1P2AValueTimeMinKNumberLimit`、`InpBSumValueMinRatioOfAValue`、`InpBSumValueMaxRatioOfAValue`、`InpPreCondPriorDeclineLookbackBars`、`InpPreCondPriorDeclineMinDropRatioOfStructure`、`InpPreCondPriorDeclineMinBarsBetweenPre0AndP0` 和 `InpP5AnchoredProfitC` 等运行时输入参数。`InpSymbols` SHALL 接受以分号分隔的交易品种列表，且运行时 SHALL 在解析后忽略空白项。`InpProfitObservationBars` SHALL 使用 bar 数量表示止盈后观察窗口长度；当其为 `0` 时，策略 SHALL 视为禁用止盈观察窗口；当其为正整数时，策略 SHALL 在对应 `symbol + timeframe` 的最近一次 `profit_target` 平仓后，按该 bar 数长度执行观察期门控。`InpStopObservationBars` SHALL 使用相同的 bar 计数语义表示止损后观察窗口长度；当其为 `0` 时，策略 SHALL 视为禁用止损观察窗口；当其为正整数时，策略 SHALL 在对应 `symbol + timeframe` 的最近一次 `hard_stop` 或 `soft_stop` 平仓后，按该 bar 数长度执行观察期门控。`InpP5AnchoredProfitC` SHALL 作为首次 `P5/P6` 激活后 `P5` 锚定止盈公式的独立系数输入。
