...
策略 SHALL 在入场时保持弱止损未激活。只有在入场后走势形成 `S_P4P5` 和 `S_P5P6` 且 `e >= n * (c + d)` 时，策略才 SHALL 激活 `soft_loss_price`，其中 `soft_loss_price = softLossC * Price_P5`。本变更对应的运行时默认参数 SHALL 将 `InpP5P6ReboundMinRatioOfP3P5Drop` 设为 `0.65`。
